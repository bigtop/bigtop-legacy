...
trait BaseUser[T <: BaseUser[T]] extends Record[T] with Loggable {
  ...
  val email = new EmailField(this, 256) {
    val emailRx = Pattern.compile("^[^@]+@[^@]+$")

    override def setFilter =
      trim _ ::
      super.setFilter

    def validateEmail(email: String): List[FieldError] =
      Option(email) match {
        case Some(email) if emailRx.matcher(email).matches => Nil
        case _ => Text("Please enter a valid email address")
